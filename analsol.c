...
FILE * diskini(int calltype)
{
  ...
  FTYPE opt,surf,teff4,sigtemp,kaptemp,dentemp,ttemp,opatemp;
  SFTYPE numvtime,tvisc;
  int outtype;
  SFTYPE rho;
  char check[50];
  ...
  if(calltype==100){
	...
	Rinner=0.2*rg;
	Router=60.*rg; // normal outer boundary HK-like
	...
	bcox3=5;
      ...
      DTtimestep = DTl*1.0 ; // how often to output dominate term in timestep to failfile
      DTdivb=DTsp = DTl*100.0 ; // how often to output sonic point info
...
//      IEFRACFLOOR=DENSITYFRACFLOOR;
//      DENSITYFLOOR=DENSITYFRACFLOOR*rho0;
 	DENSITYFLOOR=Siga/Router*1.E-11;
...
// temperature times 0.1 to simulate the viscous heating
	...
	LOOPC1{
	  opt=0.;
	  surf=0.;
	  teff4=3.*GM*5.0E-8*sunmasscgs/yrcgs/(munit/tunit)/8./M_PI/sigmacontnew/pow(radiustouse[k][1][i],3.);
	  LOOPC2{
	    tirr0[k][j][i]=pow(teff4,0.25);
	    for(m=1;m<=10;m++){
	      ...
	      opt=opt+dx[2][2][j]*G2(2,i)*(kaptemp+sigtemp);
	      surf=surf+dx[2][2][j]*G2(2,i)*0.5*(sanal[1][k][j][i]+sanal[1][k][j-1][i]);
	      if(surf<=Siga){
       	        tirr0[k][j][i]=pow(3./4.*teff4*(opt+2./3.),0.25);
	      }else{
	...
	if(l>=2){ // v_1,2,3, using different positions
      ...
      if(REENTER) {
        LOOPC{
          if(l==1){
            sanal[1][k][j][i]=extre(radiustouse[k][j][i],thetatouse[k][j][i],1);
            sanal[2][k][j][i]=extre(radiustouse[k][j][i],thetatouse[k][j][i],2);
          }
          if(l==2){vanal[1][1][k][j][i]=extre(radiustouse[k][j][i],thetatouse[k][j][i],4);}
          if(l==3){vanal[1][2][k][j][i]=extre(radiustouse[k][j][i],thetatouse[k][j][i],5);}
          if(l==4){vanal[1][3][k][j][i]=extre(radiustouse[k][j][i],thetatouse[k][j][i],6);}
        }
...
// read grid
 for(l=1;l<=2;l++){
   if(l==1){
    sprintf(namere,"./9680stop3/0_gridact1.par");
   }else{
    sprintf(namere,"./9680stop3/0_gridact2.par");
   }
...
// read data
   sprintf(namere,"./9680stop3/dump0100.dat");
   if((fpre=fopen(namere,"rt"))==NULL){
        fprintf(fail_file,"grid1: Cannot open: %s\n",namere);
        myexit(1);
   }
...
//       fprintf(stderr,"read grid %s",cptr);
   ...
   fprintf(stderr," %21.7e xrei xrej %lf %lf",datre[1][0][0][0],xre[1][1][0],xre[1][2][0]);
   fclose(fpre);
#endif
}
