...
void coolingKaistep(void)
{
  ...
  int jbdra[N1M],*jbdr;
  FTYPE der1,der2,dtdx,yp,beta,qa,qb,qc,qd,qe,coolerden,sigmaKai,dttemp,cs2,t4,dtsmallSP;
   ...
   if(thingrid[k][j][i]==0){
     ...
     (G4(1,j+1)/dx[2][2][j+1]/G2(2,i)*drt[2][k][j+1][i]*(tkai[k][j+1][i]-tkai[k][j][i])+G4(1,j)*fbdry[i])/G2(2,i)/dvl[1][2][j];
	  ...
	  coolerden=SSMALL*0.1;
          t4=fbdry[i]/4./sigmaKai*3.*(optd[k][j][i]+2./3.);
...
*/
   dttemp=0.05*s[2][k][j][i]/fabs(coolerden);
