...
void qparampi(int initmode)
{
  int i,j,k;
  FTYPE qpara2;
//  fprintf(log_file,"alpha1 %15.10g %15.10g %15.10g %15.10g \n",alpha_reala[0][-N2BND+1][10],alpha_reala[0][0][10],alpha_reala[0][N2-N2BND+1][10],alpha_reala[0][N2][10]);
  LOOPC3{
    LOOPC1{
      qpara2=gam*(gam-1.)*s[2][0][N2-1][i]/s[1][0][N2-1][i]*(1./x[2][1][i]/x[2][1][i]/x[2][1][i])/M_PI/M_PI/2./Sigar[0][N2-1][i]/2./Sigar[0][N2-1][i]/1.*MASSBH*MASSBH;
      if(initmode==0){
	if(alpha_reala[0][N2-1][i]!=alpha_real0){
          alpha_reala[0][N2][i]=exp(-qpara2);
        }
      }
      for(j=N2-1;j>=0;j--){
        if(alpha_reala[0][j][i]!=alpha_real0&&alpha_reala[0][j+1][i]!=alpha_real0){
          alpha_reala[0][j][i]=alpha_reala[0][j+1][i];
        }
      }
    }
  }
//  fprintf(log_file,"alpha2 %15.10g %15.10g %15.10g %15.10g \n",alpha_reala[0][-N2BND+1][10],alpha_reala[0][0][10],alpha_reala[0][N2-N2BND+1][10],alpha_reala[0][N2][10]);
}
...
//  fprintf(grid_file,"\n");
 if(twolayer==1){
  if(intsimp==1){
   for(i=0;i<N1;i++){
    if(initmode==0){
     Sig=Sigar[0][-N2BND][i]=0.;	
    }else{
     Sig=Sigar[0][-N2BND][i];
    }
     for(j=-N2BND+1;j<N2+N2BND;j++){
         Sig=Sigar[0][j][i]=Sig+0.5*(s[1][0][j][i]+s[1][0][j-1][i])*x[2][1][i]*(x[2][2][j]-x[2][2][j-1]);
	 cs2temp=cs2a[0][j][i]=gam*(gam-1.0)*s[2][0][j][i]/s[1][0][j][i];
...
//   fprintf(log_file,"sigma1 %15.10g %15.10g %15.10g %15.10g \n",Sigar[0][-N2BND][10],Sigar[0][-N2BND+1][10],Sigar[0][0][10],Sigar[0][1][10]);
//   fprintf(log_file,"sigma2 %15.10g %15.10g %15.10g %15.10g \n",Sigar[0][N2-N2BND][10],Sigar[0][N2-N2BND+1][10],Sigar[0][N2][10],Sigar[0][N2+1][10]);
  } else if (intsimp==0){
   for(i=0;i<N1-1;i++) {
     ijudge=0.;
     for(j=N2/2;j<N2;j++){
  ...
  if(t>=tgriddiag&&igriddiag<=10){
    for(i=0;i<N1-1;i++){
      for(j=0;j<N2;j++){
       fprintf(grid_file,"x:%10.4g y:%10.4g rho:%15.6e sig:%15.6e t:%15.6e alpha:%12.6g ",x[2][1][i]*fabs(cos(x[2][2][j])),x[2][1][i]*sin(x[2][2][j]),s[1][0][j][i],Sigar[0][j][i],tirr0[0][j][i],alpha_reala[0][j][i]);
      }   
