	   ...
	   int wcom) // 1,2,3, 0: none, 123=all 12=12 13=13, 23=23 (not currently setup to do 13 or 23 since never needed)
{
  ...
  static int si[8],sj[8],sk[8],gi[8],gj[8],gk[8];
  static int ix[8][8][4][3+1]; // ix[corner][zone in corner][i/p/o/r(a)][dir=1,2,3] (zone order such that inner corner first, then rest of 2D layer, then 3D layer last 4
  static int numedges,numedgezones;
  static int edgesi[12],edgesj[12],edgesk[12],edgegi[12],edgegj[12],edgegk[12],edgeend[12];
  static int edgeix[12][4][4][3+1]; // edgeix[edge#][zone in edge][i/p/o/r(a)][dir=1,2,3] (zone order such that inner corner first, then rest of 2D layer, then 3D layer last 4
  ...
  if(firsttime){
    if(COMPDIM==3){
      ...
      if((N1>1)&&(N2>1)&&(N3>1)){ // edges only exist in full 3D, don't do extra work
	numedges=12;
	numedgezones=4;
      }
      else{
...
#include "boundpos.h"
	...
	bi=bcs[wbound][1][k+gk[corner]][j+gj[corner]][i]; // i-boundary type
	bj=bcs[wbound][1][k+gk[corner]][j][i+gi[corner]]; // j-boundary type
	bk=bcs[wbound][1][k][j+gj[corner]][i+gi[corner]]; // k-boundary type
...
#include "boundpos.h"
	  ...
	  case 5:
	    if(divbfix==0){
	      ...
	      for(m=1;m<=3;m++){
		if(docom[m]){
		  if(periodicx2special&&(bcdim==2)&&(m==2)&&((wvec==-3)||(wvec==1)||(wvec==2)||(wvec==-1))){
		    // then on x2-edge
		    ...
		    if((bcdim==2)&&((m==2)||(m==3))&&(periodicx2special)) dosign=-1; // applies to wvec==-4 only currently
