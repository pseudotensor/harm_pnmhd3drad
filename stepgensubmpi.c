...
void coolingKaistep(void)
{
  ...
  FTYPE der1,der2,dtdx,yp,beta,qa,qb,qc,qd,qe,coolerden,dttemp,cs2,t4,dtsmallSP;
  FTYPE drta[2][N3M][N2M][N1M],tatm4a[N2M][N1M];
  FTYPE (*tt),(*dtde),(*dkapdt),(*tdr)[N2M][N1M],(*drt)[N3M][N2M][N1M];
  ...
  LOOPRC3 {
    LOOPRC2 {
      LOOPRC1 {
       ...
       tdr[k][j][i]=-16.*sigmacontnew*beta*tkai[k][j][i]*tkai[k][j][i]*tkai[k][j][i]/(kapk[k][j][i]+sigk[k][j][i]);
      }
    }
  }
  LOOPC3{
...
// DIV(F) term
  ...
  LOOPC2 {
   if(thingrid[k][j][i]==0){
	  ...
	  if(optd[k][j][i]>(2./3.)&&optd[k][j-1][i]<(2./3.)){
     ...
     (G4(1,j+1)/dx[2][2][j+1]/G2(2,i)*drt[2][k][j+1][i]*(tkai[k][j+1][i]-tkai[k][j][i])+G4(1,j)*fbdry[k][j][i])/G2(2,i)/dvl[1][2][j];
	  diag3[k][j][i]=(qa*(tkai[k][j][i+1]-tkai[k][j][i])-qb*(tkai[k][j][i]-tkai[k][j][i-1]))/dvl[1][1][i];
	  diag3[k][j+1][i]=(G4(1,j+1)/dx[2][2][j+1]/G2(2,i)*drt[2][k][j+1][i]*(tkai[k][j+1][i]-tkai[k][j][i])+G4(1,j)*fbdry[k][j][i])/G2(2,i)/dvl[1][2][j];
	  diag3[k][j+2][i]=G4(1,j+1)/dx[2][2][j+1]/G2(2,i)*drt[2][k][j+1][i]*(tkai[k][j+1][i]-tkai[k][j][i]);
          diag3[k][j+3][i]=fbdry[k][j][i];
   ...
   -(kapk[k][j][i]+sigk[k][j][i])*2.*fbdry[k][j][i]-kapk[k][j][i]*tatm4[k][j][i]*sigmacontnew;
	  ...
	  if(optd[k][j][i]<1.e-3){
            ...
            t4=fbdry[k][j][i]/4./sigmacontnew*3.*(optd[k][j][i]+2./3.);
...
*/
   dttemp=0.31623*s[2][k][j][i]/fabs(coolerden);
   diag3[k][j][i]=1./dttemp;
