...
#define OUTPUTEMFCURRENTS 1


void dump_emfcurrents(FTYPE (*magemf)[N3M][N2M][N1M], FTYPE (*sdump)[N3M][N2M][N1M], FTYPE (*vdump)[3][N3M][N2M][N1M], FTYPE (*jcurrentdump)[N3M][N2M][N1M]);
...
#if(OUTPUTEMFCURRENTS)
  dump_emfcurrents(magemf, s, v, jcurrent);
#endif
...
void nu_res_compute(void)
{
  ...
  if(rreal==1){    
    ...
    LOOPHC{ // most needed(by moc_ct)
      ...
      jz=c2z_3(jcurrent[3],k,j,i);
      // Jon+Avery version
      if(s[1][k][j][i]>1E4*DENSITYFLOOR) nu_res_real[k][j][i]=resist_real*nu_res_fact[k][j][i]*sqrt((jx*jx+jy*jy+jz*jz)/s[1][k][j][i]);
      else nu_res_real[k][j][i]=1E5*resist_real*nu_res_fact[k][j][i]*sqrt((jx*jx+jy*jy+jz*jz))/s[1][k][j][i];
    }
  }
}
...
{
  FILE *out;
  static int emfcurrentscount=0;
  static FTYPE temfcurrentsdump=0;
  char filename[200];
  int dodump;
  int i,j,k;
  FTYPE vcent[4],Bcent[4],emfcent[4],jcent[4];

  if(t>= temfcurrentsdump){
    dodump=1;
    temfcurrentsdump+=DTd;
  }
  else dodump=0;


  if(dodump){
    sprintf(filename,"emfcurrents%04d.dat",emfcurrentscount);
    out=fopen(filename,"wb");
    if(out==NULL){
      fprintf(stderr,"Can't open emfcurrents file %d\n",emfcurrentscount);
      exit(0);
    }
    ...
    LOOPC{
      ...
      if(DUMPSM==0){
	fprintf(out,"%3d %3d %3d %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g\n",i,j,k,sdump[1][k][j][i],sdump[2][k][j][i],log(sdump[1][k][j][i]),log(sdump[2][k][j][i]),vdump[1][1][k][j][i],vdump[1][2][k][j][i],vdump[1][3][k][j][i],vdump[2][1][k][j][i],vdump[2][2][k][j][i],vdump[2][3][k][j][i],magemf[1][k][j][i],magemf[2][k][j][i],magemf[3][k][j][i],jcurrentdump[1][k][j][i],jcurrentdump[2][k][j][i],jcurrentdump[3][k][j][i]);
      }
      else{
	vcent[1]=e2z_1(vdump[1][1],k,j,i);
	vcent[2]=e2z_2(vdump[1][2],k,j,i);
	vcent[3]=e2z_3(vdump[1][3],k,j,i);
	
	Bcent[1]=e2z_1(vdump[2][1],k,j,i);
	Bcent[2]=e2z_2(vdump[2][2],k,j,i);
	Bcent[3]=e2z_3(vdump[2][3],k,j,i);
	
	emfcent[1]=c2z_1(magemf[1],k,j,i);
	emfcent[2]=c2z_2(magemf[2],k,j,i);
	emfcent[3]=c2z_3(magemf[3],k,j,i);
	
	jcent[1]=c2z_1(jcurrentdump[1],k,j,i);
	jcent[2]=c2z_2(jcurrentdump[2],k,j,i);
	jcent[3]=c2z_3(jcurrentdump[3],k,j,i);
	
	fprintf(out,"%3d %3d %3d %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g %15.7g\n",i,j,k,sdump[1][k][j][i],sdump[2][k][j][i],log(sdump[1][k][j][i]),log(sdump[2][k][j][i]),vcent[1],vcent[2],vcent[3],Bcent[1],Bcent[2],Bcent[3],emfcent[1],emfcent[2],emfcent[3],jcent[1],jcent[2],jcent[3]);
      }
