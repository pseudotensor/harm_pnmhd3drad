...
#define BYTHETARANGE (0.25*M_PI)
#define BYRRANGE (1.1*L[1][1])
 
    if((analoutput==5)||(analoutput==14)){
      if(COORD==3){
				if((fabs(x[1][2][j]-0.5*M_PI)>=BYTHETARANGE)&&(x[1][1][i]>=L[1][1])&&((x[1][1][i]<=BYRRANGE)||(L[1][1]+dx[2][1][0]>=BYRRANGE)) ){
...
FTYPE e2zdiag(int whichvar, FTYPE (*var)[N2M][N1M], int k, int j, int i)
{
  ...
  if(transpassivex1||transpassivex2||transpassivex3){
    if( whichvar==BX || whichvar==BY|| whichvar==BZ ){
      return(var[k][j][i]);
    }
  }

  if(whichvar==RHO || whichvar==UU || whichvar==POT){
    return(var[k][j][i]);
  }
  else if(whichvar==UX){
    return(e2z_1(var,k,j,i));
  }
  else if(whichvar==UY){
    return(e2z_2(var,k,j,i));
  }
  else if(whichvar==UZ){
    return(e2z_3(var,k,j,i));
  }
  else if(whichvar==BX){
    return(e2z_1(var,k,j,i));
  }
...
void dump1data(int which, int whichvar, int i, int j, int k, FILE* fileptr, FTYPE var)
{
  ...
  if( (whichvar==BX)||(whichvar==BY)||(whichvar==BZ) ){
    if(transpassivex1||transpassivex2||transpassivex3){
      special=1;
      if(passivepos!=-1) which=passivepos; // otherwise in correct position
    }// else normal
  }

  if(special
#if(DO_ACCRETOR)
     || (!inside_accretor(which,i,j,k))
#else
     || 1
#endif
     ){
...
void dump(FILE * fp, int dump_cnt, int which,int outtype)
{
  ...
  FTYPE (*alpharealain)[N3M][N2M][N1M];
  ...
  if( (which==DTYPE)||(which==PDTYPE) ){
    if( (realsampled==0)||(nogridchoice==1) ){ // no interp
      ...
      if(FULLOUTPUT==0){
				...
				alpharealain=(FTYPE (*) [N3M][N2M][N1M])(&(diagna[-1][N3BND/2][N2BND/2][N1BND/2]));
      }
      else if(FULLOUTPUT==2){
				scain=(FTYPE (*) [N3M][N2M][N1M])(&(sa[-1][0][0][0]));
				vecin=(FTYPE (*) [3][N3M][N2M][N1M])(&(va[-1][-1][0][0][0]));
				alpharealain=(FTYPE (*) [N3M][N2M][N1M])(&(diagna[-1][0][0][0]));
      }
    }
  }
  else if(which==ADTYPE){
    ...
    LOOPDIAGOUTPUT(FULLOUTPUT,FULLOUTPUT,FULLOUTPUT){
      ...
      if((which==DTYPE)||(which==PDTYPE)||(which==ADTYPE)||(which==FLTYPE) ){
				if((mpicombine==0)&&(!binaryoutput)){
					for(l=1;l<=NUMSCA;l++){
					  dump1data(CENT,RHO,i,j,k,fp,scain[l][k][j][i]);
					...
					for(l=1;l<=REALNUMVEC;l++){
						if(which==FLTYPE){
							...
							dump1data(CENT,RHO,i,j,k,fp,floorvar0[l][k][j][i]);
						}
						for(m=1;m<=3;m++){
							if(dumpsmdump&&(nogridchoice==0) ){
								if(m==1){
									...
									else tempf=e2zdiag(UX+(l-1)*3+(m-1),vecin[l][m],k,j,i);
								}
								else if(m==2){
									if(j==OUTFULL2-1){
										tempf=vecin[l][m][k][j][i];
								...
								dump1data(CENT,UX+(l-1)*3+(m-1),i,j,k,fp,tempf);
							}
							else{
							  tempf=vecin[l][m][k][j][i];
							  dump1data(m,UX+(l-1)*3+(m-1),i,j,k,fp,tempf);
							}
						}
 					}
					if(actcontour==1){
					dump1data(CENT,RHO,i,j,k,fp,alpharealain[1][k][j][i]);
					dump1data(CENT,RHO,i,j,k,fp,alpharealain[2][k][j][i]);
					dump1data(CENT,RHO,i,j,k,fp,alpharealain[3][k][j][i]);
					dump1data(CENT,RHO,i,j,k,fp,alpharealain[4][k][j][i]);
					dump1data(CENT,RHO,i,j,k,fp,alpharealain[5][k][j][i]);
					dump1data(CENT,RHO,i,j,k,fp,alpharealain[6][k][j][i]);
					}
				}
...
#define WHICHDUM (dumlf)
					...
					if(!dumpsmdump){
						if(analoutput!=13){
							...
							dump1databin(CENT,RHO,i,j,k,&WHICHDUM, sizeof(FTYPE), 1, fp);
					//							fwrite(&WHICHDUM,sizeof(FTYPE),1,fp);
						}
						if(analoutput!=13){ // limited variables needed
							WHICHDUM=scain[2][k][j][i];
							dump1databin(CENT,UU,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						...
						dump1databin(VDIR1,UX,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						WHICHDUM=vecin[1][2][k][j][i];
						dump1databin(VDIR2,UY,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						WHICHDUM=vecin[1][3][k][j][i];
						dump1databin(VDIR3,UZ,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						...
						dump1databin(VDIR1,BX,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						WHICHDUM=vecin[2][2][k][j][i];
						dump1databin(VDIR2,BY,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						WHICHDUM=vecin[2][3][k][j][i];
						dump1databin(VDIR3,BZ,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						...
						if(analoutput!=13){
							...
							dump1databin(CENT,UU,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
							WHICHDUM=scain[3][k][j][i];
							dump1databin(CENT,POT,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						}
						if(analoutput!=13){
							if(which==FLTYPE){
								WHICHDUM=floorvar0[1][k][j][i];
						...
						WHICHDUM=e2zdiag(UX,vecin[1][1],k,j,i);
						dump1databin(CENT,UX,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						WHICHDUM=e2zdiag(UY,vecin[1][2],k,j,i);
						dump1databin(CENT,UY,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						WHICHDUM=e2zdiag(UZ,vecin[1][3],k,j,i);
						dump1databin(CENT,UZ,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						...
						WHICHDUM=e2zdiag(BX,vecin[2][1],k,j,i);
						dump1databin(CENT,BX,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						WHICHDUM=e2zdiag(BY,vecin[2][2],k,j,i);
						dump1databin(CENT,BY,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
						WHICHDUM=e2zdiag(BZ,vecin[2][3],k,j,i);
						dump1databin(CENT,BZ,i,j,k,&WHICHDUM,sizeof(FTYPE),1,fp);
					...
					if(analoutput!=13){
						if(!dumpsmdump){
							// rho
    						        writebuf1data(CENT,RHO,i,j,k,scain[1][k][j][i],&writebuf[BUFFERMAP]);

							// en
							writebuf1data(CENT,UU,i,j,k,scain[2][k][j][i],&writebuf[BUFFERMAP]);
							// pot
							writebuf1data(CENT,POT,i,j,k,scain[3][k][j][i],&writebuf[BUFFERMAP]);
							if(which==FLTYPE){
								writebuf1data(CENT,RHO,i,j,k,floorvar0[1][k][j][i],&writebuf[BUFFERMAP]);
							}
							// v1
							writebuf1data(VDIR1,UX,i,j,k,vecin[1][1][k][j][i],&writebuf[BUFFERMAP]);
							// v2
							writebuf1data(VDIR2,UY,i,j,k,vecin[1][2][k][j][i],&writebuf[BUFFERMAP]);
							// v3
							writebuf1data(VDIR3,UZ,i,j,k,vecin[1][3][k][j][i],&writebuf[BUFFERMAP]);
							if(which==FLTYPE){
								writebuf1data(CENT,RHO,i,j,k,floorvar0[2][k][j][i],&writebuf[BUFFERMAP]);
							}
							// BX
							writebuf1data(VDIR1,BX,i,j,k,vecin[2][1][k][j][i],&writebuf[BUFFERMAP]);
							// BY
							writebuf1data(VDIR2,BY,i,j,k,vecin[2][2][k][j][i],&writebuf[BUFFERMAP]);
							// BZ
							writebuf1data(VDIR3,BZ,i,j,k,vecin[2][3][k][j][i],&writebuf[BUFFERMAP]);
						}
						else{
							// rho
							writebuf1data(CENT,RHO,i,j,k,scain[1][k][j][i],&writebuf[BUFFERMAP]);
							...
							writebuf1data(CENT,POT,i,j,k,scain[3][k][j][i],&writebuf[BUFFERMAP]);
							// v1
							writebuf1data(CENT,UX,i,j,k,vecin[1][1][k][j][i],&writebuf[BUFFERMAP]);
							// v2
							writebuf1data(CENT,UY,i,j,k,vecin[1][2][k][j][i],&writebuf[BUFFERMAP]);
							// v3
							writebuf1data(CENT,UZ,i,j,k,vecin[1][3][k][j][i],&writebuf[BUFFERMAP]);
							// BX
#if(DIAGF)
							for(ll=1;ll<=33;ll++){ 
#else
							for(ll=1;ll<=6;ll++){		
#endif
							writebuf1data(CENT,BX,i,j,k,diagn[ll][k][j][i],&writebuf[BUFFERMAP]);
							}
						...
						if(!dumpsmdump){
							...
							// v1
						        writebuf1data(CENT,UX,i,j,k,e2zdiag(UX,vecin[1][1],k,j,i),&writebuf[BUFFERMAP]);
							// v2
							writebuf1data(CENT,UY,i,j,k,e2zdiag(UY,vecin[1][2],k,j,i),&writebuf[BUFFERMAP]);
							// v3
							writebuf1data(CENT,UZ,i,j,k,e2zdiag(UZ,vecin[1][3],k,j,i),&writebuf[BUFFERMAP]);
							// BX
							writebuf1data(CENT,BX,i,j,k,e2zdiag(BX,vecin[2][1],k,j,i),&writebuf[BUFFERMAP]);
							// BY
							writebuf1data(CENT,BY,i,j,k,e2zdiag(BY,vecin[2][2],k,j,i),&writebuf[BUFFERMAP]);
							// BZ
							writebuf1data(CENT,BZ,i,j,k,e2zdiag(BZ,vecin[2][3],k,j,i),&writebuf[BUFFERMAP]);
							// rho
							writebuf1data(CENT,RHO,i,j,k,scain[1][k][j][i],&writebuf[BUFFERMAP]);
						}
				...
				if(VISCMEM){
					...
					if(mpicombine==0){
						for(l=1;l<=6;l++){ // number of components in sym tensor
						  dump1data(CENT,RHO,i,j,k,fp,ftempT[l]);
						}
						dump1data(CENT,RHO,i,j,k,fp,nu_real[k][j][i]);	
					}
					else{
						for(l=1;l<=6;l++){ // number of components in sym tensor
							writebuf1data(CENT,RHO,i,j,k,ftempT[l],&writebuf[BUFFERMAP]);
						}
						writebuf1data(CENT,RHO,i,j,k,nu_real[k][j][i],&writebuf[BUFFERMAP]);
					}
				}// if sigma dump
				if(1){ //forces,fluxes, and energy
					if(mpicombine==0){
						for(l=1;l<=18;l++){// number of terms
							dump1data(CENT,RHO,i,j,k,fp,ffv_calcs(1,l,i,j,k));
						}
						for(l=1;l<=36;l++){// number of terms
							dump1data(CENT,RHO,i,j,k,fp,ffv_calcs(2,l,i,j,k));
						}
						for(l=1;l<=18;l++){// number of terms
							dump1data(CENT,RHO,i,j,k,fp,ffv_calcs(3,l,i,j,k));
						}
					}
					else{
						for(l=1;l<=18;l++){// number of terms
							writebuf1data(CENT,RHO,i,j,k,ffv_calcs(1,l,i,j,k),&writebuf[BUFFERMAP]);
						}
						for(l=1;l<=36;l++){// number of terms
							writebuf1data(CENT,RHO,i,j,k,ffv_calcs(2,l,i,j,k),&writebuf[BUFFERMAP]);
						}
						for(l=1;l<=18;l++){// number of terms
							writebuf1data(CENT,RHO,i,j,k,ffv_calcs(3,l,i,j,k),&writebuf[BUFFERMAP]);
						}
					}
				}
      }
				...
				if(mpicombine==0){
				  dumpcheckfprintf(CENT,RHO,i,j,k,fp,"%21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g",avg2_3[1][j][i],avg2_3[2][j][i],avg2_3[3][j][i],avg2_3[4][j][i],avg2_3[5][j][i],avg2_3[6][j][i],avg2_3[7][j][i],avg2_3[8][j][i],avg2_3[9][j][i],avg2_3[10][j][i],avg2_3[11][j][i],avg2_3[12][j][i],avg2_3[13][j][i],avg2_3[14][j][i],avg2_3[15][j][i]);
				}
				else{
					for(l=1;l<=15;l++){
						writebuf1data(CENT,RHO,i,j,k,avg2_3[l][j][i],&writebuf[BUFFERMAP]); // only applies in 2D
					}
				}
      }// if avg2d dump
				...
				if(mpicombine==0){
					dumpcheckfprintf(CENT,RHO,i,j,k,fp,"%21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g",ppcalcs[1][k][j][i],ppcalcs[2][k][j][i],ppcalcs[3][k][j][i],ppcalcs[4][k][j][i],ppcalcs[5][k][j][i],ppcalcs[6][k][j][i],ppcalcs[7][k][j][i],ppcalcs[8][k][j][i],ppcalcs[9][k][j][i],ppcalcs[10][k][j][i],ppcalcs[11][k][j][i],ppcalcs[12][k][j][i]);
				}
				else{
					for(l=1;l<=12;l++){
						writebuf1data(CENT,RHO,i,j,k,ppcalcs[l][k][j][i],&writebuf[BUFFERMAP]);
					}
				}
				...
				if(mpicombine==0){
					...
					dumpcheckfprintf(CENT,RHO,i,j,k,fp,"%.6g",ax3[k][j][i]); // assume for now don't compute anything from it, just display, so don't need all precision
				}
				else{
					writebuf1data(CENT,RHO,i,j,k,ax3[k][j][i],&writebuf[BUFFERMAP]);
				}
  ...
  //
    ...
    LOOPD{
      ...
      if(SAMEGRIDD>0){
				...
				if( (which==DTYPE)||(which==PDTYPE)||(which==ADTYPE)||(which==FLTYPE) ){
					for(l=1;l<=NUMSCA;l++){
						dump1data(CENT,RHO,i,j,k,fp,iq[l][j][i]);
					}
					for(l=1;l<=REALNUMVEC;l++){
						if(which==FLTYPE){
							dump1data(CENT,RHO,i,j,k,fp,work0iq[l][j][i]);
						}
						if( (COORD==3)&&(outtype>0) ){
							// coords go to z vs x in 2D in mapping, y is flat coord
							dump1data(CENT,RHO,i,j,k,fp,ftemp[l][1]); // value interpolated and mapped, so need ftemp
							dump1data(CENT,RHO,i,j,k,fp,ftemp[l][3]); // value interpolated and mapped, so need ftemp
							dump1data(CENT,RHO,i,j,k,fp,ftemp[l][2]); // value interpolated and mapped, so need ftemp
						}
						else{
							for(m=1;m<=3;m++){
								dump1data(CENT,RHO,i,j,k,fp,ftemp[l][m]);
							}
						}	      
					}
				}
					...
					if( (COORD==3)&&(outtype>0) ){
						...
						dump1data(CENT,RHO,i,j,k,fp,ftempT[1]);
						dump1data(CENT,RHO,i,j,k,fp,ftempT[3]);
						dump1data(CENT,RHO,i,j,k,fp,ftempT[2]);
						dump1data(CENT,RHO,i,j,k,fp,ftempT[6]);
						dump1data(CENT,RHO,i,j,k,fp,ftempT[5]);
						dump1data(CENT,RHO,i,j,k,fp,ftempT[4]);
					}
					else{
						for(l=1;l<=6;l++){ // number of components in sym tensor
							dump1data(CENT,RHO,i,j,k,fp,ftempT[l]);
					...
					dump1data(CENT,RHO,i,j,k,fp,iq[1][j][i]);  // interp of nuvisc
				}// if sigma
				else if(which==AVG2DTYPE){
					for(l=1;l<=5;l++){
						dump1data(CENT,RHO,i,j,k,fp,workiqavg[l][j][i]);
					}
					if( (COORD==3)&&(outtype>0) ){
						// coords go to z vs x in 2D in mapping, y is flat coord
						dump1data(CENT,RHO,i,j,k,fp,ftemp[1][1]); // value interpolated and mapped, so need ftemp
						dump1data(CENT,RHO,i,j,k,fp,ftemp[1][3]); // value interpolated and mapped, so need ftemp
						dump1data(CENT,RHO,i,j,k,fp,ftemp[1][2]); // value interpolated and mapped, so need ftemp
					}
					else{
						for(m=1;m<=3;m++){
							dump1data(CENT,RHO,i,j,k,fp,ftemp[1][m]);
						}
					}
					if( (COORD==3)&&(outtype>0) ){
						// mapping of r,th,phi to x,z,y leads to this ordering change
					...
					dump1data(CENT,RHO,i,j,k,fp,workiqavg[15][j][i]);  // interp of nuvisc
					...
					for(m=1;m<=10;m+=3){
						if( (COORD==3)&&(outtype>0) ){
							// coords go to z vs x in 2D in mapping, y is flat coord
							dump1data(CENT,RHO,i,j,k,fp,ftempC[m]); // value interpolated and mapped, so need ftemp
							dump1data(CENT,RHO,i,j,k,fp,ftempC[m+2]); // value interpolated and mapped, so need ftemp
							dump1data(CENT,RHO,i,j,k,fp,ftempC[m+1]); // value interpolated and mapped, so need ftemp
						}
						else{
							dump1data(CENT,RHO,i,j,k,fp,ftempC[m]);
							dump1data(CENT,RHO,i,j,k,fp,ftempC[m+1]);
							dump1data(CENT,RHO,i,j,k,fp,ftempC[m+2]);
						}
					}
					...
					for(m=1;m<=3;m+=3){
						if( (COORD==3)&&(outtype>0) ){
							...
							dumpcheckfprintf(CENT,RHO,i,j,k,fp,"%.6g ",ftempC[m+1]); // value interpolated and mapped, so need ftemp
						}
						else{
							//	      dump1data(m,i,j,k,fp,ftempC[m+2]);
							dumpcheckfprintf(CENT,RHO,i,j,k,fp,"%.6g ",ftempC[m+2]);
						}
					}
				...
				for(iii=0;iii<=dualgo;iii++){
					...
					for(sliceloop=0;sliceloop<SLICENUMBER;sliceloop++){
						...
						if(!GAMMIEIMAGE){
							...
							sprintf(ifnam,"%s%sim%01dp%04d",DATADIR,IMAGEIDIR,(ll-1)*3+(q-1)+2,im_cnts[outtype][ll]);// gets UX,UY,UZ,BX,BY,BZ
						}
