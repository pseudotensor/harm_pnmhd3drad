...
void coolingKaistep(void)
{
#if(KAICOOL)
  int i,j,k,l,istart,iend,firstthin;
  int jbdra[N1M],*jbdr;
  FTYPE der1,der2,dtdx,yp,beta,qa,qb,qc,qd,qe,coolerden,sigmaKai,dttemp;
  FTYPE drta[2][N3M][N2M][N1M],tatm4a[N2M][N1M],tenva[N1M];
  FTYPE (*tkai)[N2M][N1M],(*tt),(*dtde),(*kapk)[N2M][N1M],(*sigk)[N2M][N1M],(*dkapdt),(*optd)[N2M][N1M],(*tdr)[N2M][N1M],(*drt)[N3M][N2M][N1M],(*tatm4)[N1M],(*taoup)[N2M][N1M],(*tbdry),(*taobdry),(*fbdry),*tenv;
  drt = (FTYPE (*)[N3M][N2M][N1M])(&(drta[-1][N3BND][N2BND][N1BND]));
  coolerKai =(FTYPE (*)[N2M][N1M])(&(coolerKaia[N3BND][N2BND][N1BND]));
  tatm4 = (FTYPE (*)[N1M])(&(tatm4a[N2BND][N1BND]));
  jbdr = (int (*))(&(jbdra[N1BND]));
...
// y, beta and dr term  
  ...
  LOOPRC3 {
    LOOPRC2 {
      ...
      absorp_(&tt[-N1BND],&s[1][k][j][-N1BND],&istart,&iend,&kapk[k][j][-N1BND],&dkapdt[-N1BND]);
      scatt_(&tt[-N1BND],&s[1][k][j][-N1BND],&istart,&iend,&sigk[k][j][-N1BND]);
      LOOPRC1 {
       ...
       beta=(2.+yp)/(6.+3.*yp+yp*yp);
       tdr[k][j][i]=-16.*sigmaKai*beta*tkai[k][j][i]*tkai[k][j][i]*tkai[k][j][i]/(kapk[k][j][i]+sigk[k][j][i]);
      }
    }
  }
  LOOPC3{
...
//     fbdry[N1]=0.;
     LOOPC1{
  	 ...
  	 if(optd[k][j][i]<(2./3.)){
	   thingrid[k][j][i]=1;
	   tatm4[j][i]=tatm4[j-1][i]+4.*kapk[k][j][i]*tkai[k][j][i]*tkai[k][j][i]*tkai[k][j][i]*tkai[k][j][i]*dx[1][2][j]*G2(2,i);
	 }else{
	   thingrid[k][j][i]=0;
	 }
       }
       firstthin=1;
       for(j=N2-1;j>=0;j--){
	 if(thingrid[k][j][i]==1&&firstthin==1){
	   taoup[k][j][i]=(kapk[k][j][i]+sigk[k][j][i])*dx[1][2][j]*G2(2,i)/2.;
	   firstthin=0;
	   if(j==N2-1){
		fprintf(stderr,"warning: outer disk optically thin");
           }else{
	     tbdry[i]=tkai[k][j+1][i];
	     taobdry[i]=optd[k][j+1][i];
	     jbdr[i]=j+1;
	     fbdry[i]=4.*sigmaKai*(tbdry[i]*tbdry[i]*tbdry[i]*tbdry[i]-tenv[i]*tenv[i]*tenv[i]*tenv[i]*exp(-2./3.))/3./(taobdry[i]+2./3.); // here assume optical depth at different wavelength are the same,gray case, otherwise after tenv[i] there should be exp(-tao)
	   }
         }else{
	   if(thingrid[k][j][i]==1&&firstthin==0){
...
// DIV(F) term
  ...
  LOOPC2 {
   if(thingrid[k][j][i]==0){
     ...
     (G4(1,j+1)/dx[2][2][j+1]/G2(2,i)*drt[2][k][j+1][i]*(tkai[k][j+1][i]-tkai[k][j][i])+G4(1,j)*fbdry[i])/G2(2,i)/dvl[1][2][j];
	  }else{
            coolerden =
       ...
       (qd*(tkai[k][j+1][i]-tkai[k][j][i])-qe*(tkai[k][j][i]-tkai[k][j-1][i]))/qc/dvl[1][2][j];
    	  ...
    	  if(fbdry[i]<0.){fbdry[i]=0.;}
  	  coolerden = 4.*kapk[k][j][i]*sigmaKai*tkai[k][j][i]*tkai[k][j][i]*tkai[k][j][i]*tkai[k][j][i]
   -kapk[k][j][i]*sigmaKai*tenv[i]*tenv[i]*tenv[i]*tenv[i]*exp(-optd[k][j][i])
   -(kapk[k][j][i]+sigk[k][j][i])*2.*fbdry[i]-kapk[k][j][i]*tatm4[j][i]*sigmaKai;
//+sigmaKai*tatm4[i])*exp(-taoup[k][j][i])
//   -kapk[k][j][i]*tatm4[i]*sigmaKai; // assume opacity are the same at different wavelength, gray case
//	  coolerden=1.e-25;
//	  s[2][k][j][i]=pow(fabs(fbdry[i])/sigmaKai,0.25)*s[1][k][j][i]*rhounit*kcontcgs/mmw/mhydrcgs/(gam-1.)/enedenunit;
   }
   diag1[k][j][i]=alpha_reala[k][j][i];
   diag2[k][j][i]=optd[k][j][i];
   if(thingrid[k][j][i]==0){
      diag3[k][j][i]=qd*(tkai[k][j+1][i]-tkai[k][j][i])/G2(2,i)/G4(1,j+1);
   }else{
      diag3[k][j][i]=-fbdry[i];
   }
   dttemp=0.8*s[2][k][j][i]/fabs(coolerden);
   ...
   s[2][k][j][i]-=coolerKai[k][j][i]*dt;
    ...
    if(FORCEIEINTERNAL){
      if(s[2][k][j][i]<IEFLOOR){
        ...
        // normal case
        coolerie=coolerKai[k][j][i]*volume; // full is ok then
      }
    }
    else{// have to check if no floor, or might get floating point exceptions!   
      if(s[2][k][j][i]<=0.0){
