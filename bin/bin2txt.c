...
// standard compile                    gcc -O2 -Wall -o bin2txt bin2txt.c -lmfhdf -ldf -ljpeg -lz -lv5d
         ...
         )
{
  ...
  float ProjArgs[10];               /* the projection parameters */
  ...
  if(argc<11){
    ...
    fprintf(stderr," BLE: same endian (0), big to little or little to big (same process) (1)\n");
  ...
  if((source==3)||(dest==3)||(source==4)||(dest==4)){ // use maximal holder array (doubles)
    if(HDFTYPE==DFNT_UINT8){
      arrayi=(unsigned char*)malloc(sizeof(unsigned char)*numcolumns*N1*N2*N3); // (DIM0->N1, DIM1->N2 where array[DIM0][DIM1])
      array=arrayi;
    }
  ...
  for (it=0;it<TS;it++) { // loop over timeslices
    ...
    if((dest==5)&&(it==0)){ // assume all output vis5d are multi-timed, so only open 1 file for all timeslices
      fprintf(stderr,"Openining vis5d file\n"); fflush(stderr);
  ...
  for(i=0;i<numrows;i++){
	...
	case 1:
	  ...
	  break;
	case 2:
	  if(precision[j]=='b')	            fscanf(input,"%c",&dumb);
	...
	case 1:
	  ...
	  if(precision[j]=='i'){if(ble){swap(&dumi,intsize);}     fwrite(&dumi,intsize,1,output);}
	  if(precision[j]=='f'){if(ble){swap(&dumf,floatsize);}   fwrite(&dumf,floatsize,1,output);}
	  if(precision[j]=='d'){if(ble){swap(&dumlf,doublesize);} fwrite(&dumlf,doublesize,1,output);}
	  break;
	case 2:
	  if(precision[j]=='b'){
	  ...
	  if(precision[j]=='i'){if(ble){swap(&dumi,intsize);}     fprintf(output,"%d ",dumi);}
	  if(precision[j]=='f'){if(ble){swap(&dumf,floatsize);}   fprintf(output,"%15.8g ",dumf);}
	  if(precision[j]=='d'){if(ble){swap(&dumlf,doublesize);} fprintf(output,"%21.15g ",dumlf);}
	  break;
#if(HDF)
	case 3:
	case 4:
	  if(precision[j]=='b') arrayb[nextbuf++]=dumb;
	  if(precision[j]=='i'){if(ble){swap(&dumi,intsize);}     arrayi[nextbuf++]=dumi;}
	  if(precision[j]=='f'){if(ble){swap(&dumf,floatsize);}   arrayf[nextbuf++]=dumf;}
	  if(precision[j]=='d'){if(ble){swap(&dumlf,doublesize);} arrayd[nextbuf++]=dumlf;}
	  break;
#endif
#if(V5D)
	case 5:
	  ...
	  if(precision[j]=='i'){if(ble){swap(&dumi,intsize);}      arrayvisf[nextbuf++]=dumi;}
	  if(precision[j]=='f'){if(ble){swap(&dumf,floatsize);}    arrayvisf[nextbuf++]=dumf;}
	  if(precision[j]=='d'){if(ble){swap(&dumlf,doublesize);}  arrayvisf[nextbuf++]=dumlf;}
	  break;
#endif
	default:
	  break;
