  ...
  if( (which==DTYPE)||(which==PDTYPE) ){
    if( (realsampled==0)||(nogridchoice==1) ){ // no interp
      ...
      if(FULLOUTPUT==0){
				...
				alpharealain=(int (*) [N2M][N1M])(&(thingrida[N3BND/2][N2BND/2][N1BND/2]));
      }
      else if(FULLOUTPUT==2){
				scain=(FTYPE (*) [N3M][N2M][N1M])(&(sa[-1][0][0][0]));
				vecin=(FTYPE (*) [3][N3M][N2M][N1M])(&(va[-1][-1][0][0][0]));
				alpharealain=(int (*) [N2M][N1M])(&(thingrida[0][0][0]));
      }
    }
  }
  else if(which==ADTYPE){
				...
				if((mpicombine==0)&&(!binaryoutput)){
					...
					for(l=1;l<=REALNUMVEC;l++){
						...
						for(m=1;m<=3;m++){
 						...
 						if(actcontour==1){
						dump1data(CENT,RHO,i,j,k,fp,alpharealain[k][j][i]);
						}
					}
				}
...
#define WHICHDUM (dumlf)
					...
					if(analoutput!=13){
						if(!dumpsmdump){
							...
							writebuf1data(CENT,POT,i,j,k,scain[3][k][j][i],&writebuf[BUFFERMAP]);
							// v1
							writebuf1data(CENT,UX,i,j,k,e2z_1(vecin[1][1],k,j,i),&writebuf[BUFFERMAP]);
							// v2
							writebuf1data(CENT,UY,i,j,k,e2z_2(vecin[1][2],k,j,i),&writebuf[BUFFERMAP]);
							// v3
							writebuf1data(CENT,UZ,i,j,k,e2z_3(vecin[1][3],k,j,i),&writebuf[BUFFERMAP]);
							// BX
							writebuf1data(CENT,BX,i,j,k,e2z_1(vecin[2][1],k,j,i),&writebuf[BUFFERMAP]);
							// BY
							writebuf1data(CENT,BY,i,j,k,e2z_2(vecin[2][2],k,j,i),&writebuf[BUFFERMAP]);
							// BZ
							writebuf1data(CENT,BZ,i,j,k,e2z_3(vecin[2][3],k,j,i),&writebuf[BUFFERMAP]);
						}
					}
					else{
						if(!dumpsmdump){
